...
router.post('/url', jwtToken, async (req, result) => {
    ...
    await client.query(`select id, big_url, short_url, created_at, title, description, clicks from url where email = $1 order by created_at desc`, [req.token.email], async function (err, res) {
        if (err) {
